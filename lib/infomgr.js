...
infomgr.queryInfo = async function(id, source, category){
  let query = {};//如果不传参数,query为空，则查询整个collection
...
infomgr.updateInfo = async function(id, year, month, day, source, category, subcategory, title, subtitle, content, remark, level, option = {}){
  if(!isDateLegal(year, month, day)){
    throw new Error(message.date_illegal);
  }

  let result;
  objid = kqudie.String2ObjectId(id);
  query_json = {
    '_id': objid
  }
  update_json = {
...
infomgr.query_detail(database, info_collection, query_json){
