...
infomgr.updateInfo = async function(modify_id, year, month, day, source, category, subcategory, title, subtitle, content, remark, level, option = {}){
  if(!isDateLegal(year, month, day)){
    throw new Error(message.date_illegal);
  }

  let result;
  objid = kqudie.String2ObjectId(modify_id);
  query_json = {
    '_id': objid
  }
  update_json = {
